...
jobs <- read_csv("jobs_data.csv", show_col_types = FALSE) %>% clean_names()
labor <- read_csv("labor_hours.csv", show_col_types = FALSE) %>% clean_names()
customers <- read_csv("customers.csv", show_col_types = FALSE) %>% clean_names()
business_unit <- read_csv("business_units.csv", show_col_types = FALSE) %>% clean_names()
region <- read_csv("regions.csv", show_col_types = FALSE) %>% clean_names()
```
...
```{r}
# CHUNK 21 — Baseline and Augmented Coefficients by Horizon
...
dc_coefficients_by_horizon %>%
  ...
  kable(
    caption = "Data Center Regression Coefficients by Horizon"
  ) %>%
  kable_styling(full_width = FALSE)
```
...
# =====================================================================
# METHODOLOGY UPGRADE — Deck-Ready Section
# =====================================================================

This section re-fits every sector (Data Center, Warehouse, Apartment, Office, Manufacturing) using a stricter, pre-specified methodology and produces the single cross-sector table for the deck. The sector-specific chunks above are kept for continuity and for showing Sandy how we got here; the **numbers on the deck come from the tables in this section**.

### What this section adds

1. **Naive and Seasonal Naive benchmarks** (`y_hat = labor_hours_lag1`,
   `y_hat = labor_hours_lag4`). We have to beat these first. If the Baseline
   doesn't beat the Naive, the problem is the target series itself, not the
   indicators.
2. **COVID structural-break dummy** on the 2020 Q1–Q4 quarters, so the
   COVID shock isn't absorbed into other coefficients.
3. **Warehouse outlier removal** (z-score > 2 on labor_hours) — the 2017/2018
   quarters we flagged at midterm.
4. **Baseline = AR(1) + COVID** instead of AR(1) + qtr. Quarterly seasonal
   dummies were tested in every sector and were generally not significant;
   we keep them only in the Full Augmented model for reference.
5. **Augmented Parsimonious (Levels)** — Baseline plus the single external
   indicator with the strongest |correlation| with labor_hours on the
   training set at that specific lag. The selection rule is pre-specified
   and applied uniformly across all sectors and horizons.
6. **Augmented Parsimonious (Change-in)** — same, but using the quarterly
   first difference of the indicator. This is the fallback for sectors
   where labor_hours and the best indicator share a strong upward trend
   (Warehouse, Apartment).
7. **Augmented Full** — all indicators + qtr + COVID (over the regressor
   budget, reference only).
8. **Horizons extended to 1Q-6Q.** The deck takes the **longest horizon at
   which some Augmented variant beats the Baseline**, with 3Q Ahead as the
   minimum actionable bar (Lithko's approval + mobilization cycle ≈ 2
   quarters).
9. **Honest no-signal fallback.** If no horizon 3Q-6Q beats the Baseline in
   either variant, we report honestly: "no reliable early macro signal
...
The table above shows the best model chosen for each sector using two rules:

1. The model must improve over the baseline forecast, and
2. The model must keep MAPE at or below 30%.

If a sector does not meet this standard at any 3Q to 6Q horizon, it is labeled as No actionable signal. That means the external indicators did not improve forecasting enough to support planning decisions at this stage.

### Additional testing for Office and Apartment
The next three chunks focus only on Office and Apartment, because these were the weakest sectors in the earlier results.
We test two practical improvements:

1. **post_2020 shift variable:**/n
    This is a dummy variable equal to 1 from 2020. It helps capture a lasting structural change that the one-year COVID dummy may miss.
2. **log transform of labor hours:**/n
    This helps when labor hours vary a lot over time. It can reduce forecast error and improve stability. After prediction, the values are converted back to labor hours so the results stay easy to interpret.
...
-   **Data Center** should pick 3Q via Aug Full (`dc_spending_lag + the
    other three`) — MAPE 7.6%, lift ~90% vs Baseline.
-   **Warehouse** picks 6Q via Aug Full — MAPE 21.2%, lift ~40%. The
    Levels spec overfits because of trend collinearity; Full recovers.
-   **Apartment**: if the 4Q Aug Levels (`undcon5musa_lag4`) wins at
    MAPE 9.7%, that is the deck pick; U11 may offer a stronger log or
    post_2020 combo.
-   **Office** is the weakest sector: short history, low
    labor-hours-vs-indicator correlation. Report the best-available
    MAPE; if no spec meets the 30% cap, the deck should say "signal too
    weak to plan against at current data depth" — an honest finding.
-   **Manufacturing** should pick 5Q or 6Q via Aug Levels (`pnfi_lag`),
    MAPE ~5-11%, lift ~40-85%.
...
This section is only for Warehouse.
If one of these versions performs better than the current warehouse result, then that improved version can be used in the final presentation.
